...
class UsuarioRegistroControleTest {
    ...
    @Test
    @WithMockUser
    @DisplayName("JUnit test - deve atualizar um usuario e retornar status code 204")
    void deveAtualizarUmUsuarioERetornarStatusCode204() throws JsonProcessingException, Exception {

        // Given / Arrenge
        given(servico.findByCpf()).willReturn(DadosUsuario.getUsuario(usuarioDto));
        given(servico.update(any(UsuarioUpdateDto.class))).willReturn(DadosUsuario.getUsuario(usuarioDto));
